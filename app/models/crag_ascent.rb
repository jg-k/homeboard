class CragAscent < ApplicationRecord
  ...
  scope :logged_by, ->(user) { joins(:activity_log).where(activity_logs: { user_id: user }) }
...
#  thecrag_route_id  :string
#  ukc_route_id      :string
#
# Indexes
#
#  index_crag_ascents_on_thecrag_ascent_id  (thecrag_ascent_id) UNIQUE WHERE thecrag_ascent_id IS NOT NULL
#  index_crag_ascents_on_thecrag_route_id   (thecrag_route_id) WHERE thecrag_route_id IS NOT NULL
